...
func TestMatches(t *testing.T) {
	...
	em := func(s, h string, ipv6 bool) {
		m := ml(s, h, "%host%", true)
		if h != m.host {
			t.Errorf(`%s: expected host "%s", got "%s"`, s, h, m.host)
		}
		if ipv6 != m.ipv6 {
	...
	ml("invalid 4.6", "192.168.0.", "%host%", false)
	ml("invalid 4.7", "192.168.1.1", "%host% extra", false)
	...
	ml("valid 4.10", "prefix 192.168.1.1", "prefix.*?%host%", true)
	ml("valid 4.11", "192.168.1.1 suffix", "%host%.*?suffix", true)

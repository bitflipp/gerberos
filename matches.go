...
func newMatch(l string, t time.Time) match {
	var host string
	ipv6 := false
	...
	if m4 == nil {
		...
		if m6 == nil {
			return match{err: errors.New("no IPv4 or IPv6 host")}
		} else {
			if len(m6) > 1 {
				return match{err: errors.New("multiple IPv6 hosts")}
			}
			host = m6[0]
			ipv6 = true
		}
		...
		if len(m4) > 1 {
			return match{err: errors.New("multiple IPv4 hosts")}
		}
		host = m4[0]
	}

	return match{time: t, host: host, ipv6: ipv6}
}
